...
Simplify the frontend: Replace custom chat components with Vercel AI SDK UI hooks
Modernize the backend: Replace direct Anthropic SDK calls with AI SDK Core's unified API
...
Access & Resources:
Open source project: https://github.com/replicatedhq/chartsmith
...
Keep the existing system prompts and behavior (user roles, chart context, etc.)
All existing features continue to work (tool calling, file context, etc.)
Tests pass (or are updated to reflect new implementation)

Nice to Have:
Demonstrate easy provider switching (show how to swap Anthropic for OpenAI)
Improve the streaming experience using AI SDK optimizations
Simplify state management by leveraging AI SDK's built-in patterns
...
Pull Request into the replicatedhq/chartsmith repo
Documentation:
...
Demo Video (quick Loom or similar):
